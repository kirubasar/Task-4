...
const convertToTitleCaps = function(array) {
...
const medianValues = function(arr2, arr3)  {
    const mergedArray = arr2.concat(arr3).sort((a, b) => a - b);
    const mid = (arr2.length + arr3.length) / 2;
    return (mergedArray[Math.floor(mid)] + mergedArray[Math.ceil(mid)]) / 2;
};
const array2 = [1, 2, 3, 8];
const array3 = [9, 5, 7, 4];

console.log(medianValues(array2, array3));
console.log("--------------------");
